...
class UpdateUser {
  ...
  async updatePassword(req: Request, res: Response) {
    try {
      ...
      await user.updateOne(Collections.User, { Id: Id, Email: Email }, { Password: new Hash(user.paramPassword()).hash() });
      user.flush();
  ...
  async updateGroup(req: Request, res: Response) {
    try {
      ...
      const user = new User();
      user.paramId(req.body.user);
      user.paramModifiedBy(req.body.Id);
      await user.connectDb();
      await user.checkNotExists();
      await user.updateOne(Collections.User, { Id: user.paramId() }, { UserGroup: req.body.userGroup });

      let response = new ResponseClass(
        ResStatus.Success,
        UserFieldsMessage.UserUpdated
      );
      response.setData((await user.getUser()));
      user.flush();
      return res.status(ResStatus.Success).send(response.getResponse());
    } catch (error) {
      if (error instanceof ResponseClass) {
